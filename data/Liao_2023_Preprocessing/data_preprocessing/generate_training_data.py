...
if __name__ == "__main__":
    ...
    data_files = [
        "BS11504A_S1_splicing_analysis.csv.gz",
        "BS11505A_S2_splicing_analysis.csv.gz",
        "BS11506A_S3_splicing_analysis.csv.gz",
    ]
    ...
    # For exons with multiple barcodes, keep just one.
    _preferred_barcodes = frozenset([
        'AACAAGAGATCACAAAACAA', 'AATGAATCACCATCCAGGAC', 'ACCCACCCACGATACAACAT',
        'ACCGGTACTCATTAACGTAG', 'AGACCTTCCTAAGTCCCCAC', 'AGATAAGTGTACCAAAACAA',
        'AGATACAGACTTACGATAGA', 'AGTCTACAACCTGTAACACC', 'CAATACCGACTAAAAAACGC',
        'CACCACAGACATATACGCAG', 'CAGATACACGCACCTCTGAG', 'CATACACGACCCACTTAAAC',
        'CGCCCAATCCTCCATCCTAA', 'CGCCGTAAGCGTTCGCATCC', 'CGGCATGGCAGCGCATACTC',
        'CTACTGAACACCACTGAACA', 'CTATTAGGAGACCACTTTCC', 'CTGCCTGAACGCTCTTATTT',
        'GAGCTTAAGCCATACGAATC', 'GATTAATAACCTTTTTGAAG', 'GGAAAGCAACGACGTTATTT',
        'TAAATACACATCAAGAGTGA', 'TACTAGATTTGTCACTATAA', 'TAGTTCGCTCGCTACCCACG',
        'TCACCTTAAACCATCGGTAT', 'TCAGACCAGACACACGTCAT', 'TCAGAGGACAGAACACGCAC',
        'TCCCAATTGTCTGAATAAAC', 'TCTCAGTCACATCTGGTATA', 'TCTCTCGCACTGACCTCAGT',
        'TGCACACTTCACCGTTGTGA', 'TGTCCCACCTAGACGATCTC', 'TGTTATCGATAAGCTCCTCA',
        'TTATAAGGATAGACCAGTTC', 'TTCAGGACCCCCCATAAGAA', 'TTTTCATTCTCCAACGTCAA',
    ])
    _dup_mask = dataset.duplicated("exon", keep=False)
    dataset = dataset[~(_dup_mask & ~dataset.index.isin(_preferred_barcodes))]
    dataset = dataset.drop_duplicates(subset="exon", keep="first")
    ...
    for D, Dn in tqdm(
        ...
        dump(D, output_folder / f"{Dn}_ES7_HeLa_ABC.pkl.gz")
    print('Done.')

    print('Writing exon/PSI CSVs... ', end='')
    csv_columns = [
        "exon", "num_exon_inclusion", "num_exon_skipping",
        "num_intron_retention", "num_splicing_in_exon", "num_bad_exon1",
        "num_bad_reads", "num_unknown_splicing", "total_noncanonical", "total", "PSI",
    ]
    for split_df, split_name in [(dataset_tr, "train"), (dataset_te, "test")]:
        csv_df = split_df[
            ["exon", "num_exon_inclusion", "num_exon_skipping",
             "num_intron_retention", "num_splicing_in_exon", "num_bad_exon1",
             "num_bad_reads", "num_unknown_splicing", "others", "total"]
        ].copy().rename(columns={"others": "total_noncanonical"})
        csv_df["PSI"] = csv_df["num_exon_inclusion"] / (csv_df["num_exon_inclusion"] + csv_df["num_exon_skipping"])
        csv_df[csv_columns].to_csv(
            output_folder / f"exon_psi_{split_name}_ES7_HeLa_ABC.csv.gz", index=True
        )
    print('Done.')
